package com.github.tacticaldevmc.remotecore.remotecore.config;

import com.github.tacticaldevmc.remotecore.remotecore.RemoteCoreSpigot;
import com.github.tacticaldevmc.remotecore.remotecore.config.exceptions.InvalidWorldException;
import org.bukkit.*;
...
public class Config {
    ...
    public Config(String name, boolean isDef) {
        this(RemoteCoreSpigot.getInstance().getDataFolder(), name, isDef);
    }

    public Config(String name) {
        this(RemoteCoreSpigot.getInstance().getDataFolder(), name, true);
    }
    ...
    public Location getLocation(final String path, final Server server) {
    ...
    public boolean reload() {
        if (!this.folder.exists()) {
            try {
                if (this.folder.mkdir()) {
                    RemoteCoreSpigot.getInstance().getLogger().log(Level.INFO, "- Created a new configuration file!");
                } else {
                    RemoteCoreSpigot.getInstance().getLogger().log(Level.INFO, "- §cUnable to create folder " + this.folder.getName());
                }
            } catch (Exception e) {
                RemoteCoreSpigot.getInstance().getLogger().log(Level.INFO, "Failed to reload " + this.fileName + " config.yml.");
                return false;
            }
        }
        ...
        if (this.def) {
            InputStream defaultConfigStream = RemoteCoreSpigot.getInstance().getResource(this.fileName);
...
//            Logger.WARNING.log("Failed to reload " + this.fileName + " config.yml.");
//        }
//
//        if (this.def) {
...
//    }
    ...
    public boolean save() {
        ...
        try {
            ...
            RemoteCoreSpigot.getInstance().getLogger().log(Level.INFO, ("Failed to save config.yml to " + this.configFile.getName()));
            e.printStackTrace();
            return false;
        }
    }
    ...
    private void saveDefaultConfig() {
        if (this.configFile == null)
            this.configFile = new File(RemoteCoreSpigot.getInstance().getDataFolder(), this.fileName);

        if (!this.configFile.exists()) RemoteCoreSpigot.getInstance().saveResource(this.fileName, false);
    }
}
