package com.github.tacticaldevmc.remotecore.messages.enums;

import com.github.tacticaldevmc.remotecore.ICore;
...
public enum RemoteMessages {
    ...
    public String toString() {
        ...
        return Colors.format(messageHandler.getConfig().getConfiguration().getString(path)).replace("%prefix%", settings.getPrefix());
    }
    ...
    public void send(CommandSender sender, String replacement) {
        ICore core = RemoteCoreSpigot.getInstance();
        ISettings settings = core.getSettings();
        sender.sendMessage(toString().replace("%s", Colors.format(replacement)).replace("%prefix%", settings.getPrefix()));
    }
